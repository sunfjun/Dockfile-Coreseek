...
void AddHit(csfHelper_HitCollectorObject *self, SphWordID_t iWordID, int iPos, int iPhrase)
{
	...
	self->m_pSource->AddHit( self->m_pSource->m_tDocInfo.m_iDocID, iWordID, iPos);
}
...
PyObject *PyNewHitCollector(CSphSource* pSource, const CSphString & aFieldName, int iField)
{
...
PyMODINIT_FUNC initCsfHelperWithModule (PyObject* m)
{
	csfHelper_HitCollectorType.tp_new = PyType_GenericNew;
	if (PyType_Ready(&csfHelper_HitCollectorType) < 0)
		return;
